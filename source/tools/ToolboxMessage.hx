...
class ToolboxMessage extends MusicBeatSubstate {
    ...
    public override function new(title:String, text:String, buttons:Array<Button>, ?bgColor:FlxColor, ?camera:FlxCamera) {
        ...
        for(i=>button in buttons) {
            ...
            // no butt
            var leButton = new FlxUIButton(0, y, button.label, function() {
                close();
                button.onClick(this);
            });
            leButton.x = Std.int(10 + (maxButtonWidth * (i + 0.5))) + (i * 10);
            leButton.resize(Std.int(maxButtonWidth), 20);
            leButton.x -= leButton.width / 2;
            tab.add(leButton);
        }
    ...
    public static function complexMessage(title:String, text:String, buttons:Array<Button>, ?camera:FlxCamera) {
        return new ToolboxMessage(title, text, buttons, camera);
    }
}