...
class MasterEditorMenu extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var leText:Alphabet = new Alphabet(150, 320, options[i], true);
			leText.isMenuItem = true;
			leText.targetY = i - curSelected;
			grpTexts.add(leText);

			var maxWidth = 980;
			if (leText.width > maxWidth)
			{
				leText.scaleX = maxWidth / leText.width;
			}
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			switch(options[curSelected]) {
				...
				/*#if (MODS_ALLOWED && desktop)
				case 'Discord Rich Presence Editor':
					MusicBeatState.switchState(new DiscordRichPresence());
					//openSubState(new DiscordRichPresence());
				#end*/
				default:
					MusicBeatState.switchState(new MainMenuState());
			}
