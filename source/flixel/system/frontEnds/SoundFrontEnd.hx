...
class SoundFrontEnd
{
	...
	public var timeSinceLast:Float = 0;
	public var ammountVolume:Float = 0.01;
	public function update(elapsed:Float):Void
	{
		...
		if (timeSinceLast > 0) timeSinceLast -= elapsed;
		var pressMult:Int = 1;

		#if FLX_KEYBOARD
		if (timeSinceLast > 0.4)
            pressMult = 3;
        else if (timeSinceLast > 0.3)
            pressMult = 2;
        else pressMult = 1;
		...
		if (FlxG.keys.anyJustReleased(muteKeys))
            ...
            holdTime = 0;
            changeVolume(ammountVolume * pressMult);
        }
        ...
        if (FlxG.keys.anyPressed(volumeUpKeys) || FlxG.keys.anyPressed(volumeDownKeys))
        {
            ...
            {
                var val = ((checkNewHold - checkLastHold) * pressMult) / 100;
                if (FlxG.keys.anyPressed(volumeDownKeys)) val = -val;
                changeVolume(val);
            }
        }
		#end
